...
const Update_event = () => {
  ...
  const fetchData = async (e)=>{
    ...
    let Fdata = await axios.put(BACKEND_COMMAN_URL+'/api/editEvent/'+id,formData);
    if(Fdata.status === 200){
      navigate('/View_event');
    }
    else{
  ...
  const fetchData2 = async ()=>{
    const data = await axios.get(`${BACKEND_COMMAN_URL}/api/singleEvent/${id}`);
    if(data.status === 200){
     ...
     <div className="col-12">
      ...
      <div className="col-12">
        ...
        <div className="col-6 mb-4 px-3">
          ...
          <input className="form-control" type="time" name="time" id='time' value={data.time} onChange={(e)=> setData({...data,time : e.target.value})} placeholder="Time" />
        </div><br />
        <div className="col-6 mb-4 px-3">
